...
def parser_args():
    ...
    parser.add_argument('--model_path', type=str, default='pre-train-model/test.pth',
                        help='path of checkpoint of test model')
    parser.add_argument('--baseline_path', type=str, default='pre-train-model/baseline.pth',
                        help='path of checkpoint of baseline')

    parser.add_argument('--LR_path', type=str, default='D:\\Works\\Files\\RTC_2020\\RTC_data\\interpolate\\test_3\\LR',
                        help='path of the LR images')
    parser.add_argument('--HR_path', type=str, default='D:\\Works\\Files\\RTC_2020\\RTC_data\\interpolate\\test_3\\HR',
                        help='path of the HR images')
...
def evaluation(opt):
    ...
    baseline_model.eval()
    x = torch.randn((1, 3, 240, 360)).cuda()
    summary(baseline_model, input_size=(3, 224, 224), batch_size=-1)
    ...
    bmacs = profile(baseline_model.to('cuda'), inputs)
    print('test:{:.4f} G'.format(macs / 1e9))
    print('baseline:{:.4f} G'.format(bmacs / 1e9))
    if (macs / 1e9) > 2.0:
    ...
    print('test model: {:.4f}, base model: {:.4f}, psnr: {:.4f}'.format(test_psnr, baseline_psnr,
                                                                        alpha * (test_psnr - baseline_psnr)))
    print('test model: {:.4f}, base model: {:.4f}, ssim: {:.4f}'.format(test_ssim, baseline_ssim,
                                                                        beta * (test_ssim - baseline_ssim)))
    print('test model: {:.4f} ms, base model: {:.4f} ms, time: {:.4f} ms'.format(avg_test_time * 100,
                                                                                 avg_baseline_time * 100,
                                                                                 avg_time_score))
